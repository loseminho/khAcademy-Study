...
SELECT CLASS_CODE, STUDENT_NO, CLASS_NAME,STUDENT_NAME, TEACHER_NAME
FROM APP_CLASS
...
CREATE TABLE CLASS_TBL(
    ...
    CLASS_TITLE VARCHAR2(20)    NOT NULL
);
--직원
CREATE TABLE STAFF(
    STAFF_CODE  NUMBER          PRIMARY KEY,
    STAFF_BUY   VARCHAR2(50)    NOT NULL
);
--직급
CREATE TABLE ADEPT(
    ADEPT_NO    NUMBER PRIMARY KEY,
    ADEPT_NAME  VARCHAR2(20) UNIQUE NOT NULL
);
--고객
CREATE TABLE CLIE(
    CLIE_NO         NUMBER PRIMARY KEY,
    CLIE_NAME       VARCHAR2(20) NOT NULL,
    ADEPT_NO        NUMBER REFERENCES ADEPT ON DELETE CASCADE,
);


--식별- 그 테이블에 단한번밖에 못들어감.
--비식별
