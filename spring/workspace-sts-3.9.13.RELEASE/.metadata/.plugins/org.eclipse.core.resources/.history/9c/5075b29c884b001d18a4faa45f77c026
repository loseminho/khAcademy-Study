...
<body>
	...
	<c:choose>
		<c:when test="${empty sessionScope.m }">
			...
			<a href="/searchMyPasswordFrm.do">비밀번호 찾기</a>
		...
		<c:otherwise>
			...
			<h3><a href="/dmMain.do">쪽지함 가기</a></h3>
			...
			<p>
			아이디만 입력하고 검색하는 경우 아이디로 조회,
			이름만 입력하고 검색하는 경우 이름으로 조회,
			둘 다 입력하고 검색하는 경우 두개 and로 조회
		...
		$("#allMemberAjax").on("click",function(){
			$.ajax({
				...
				success : function(data){
					...
					const table = $("<table>");
					const titleTr = $("<tr>");
					titleTr.html("<th>번호</th><th>아이디</th><th>이름</th>");
